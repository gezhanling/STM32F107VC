...
volatile uint16_t ADC_RCVTab ;   //自己添加
// extern __IO u16 ADC_ConvertedValue;
 uint16_t ADC_ConvertedValue[112];   //自己添加
...
//	DMA_InitStructure.DMA_BufferSize = 16;//设置DMA在传输时缓冲区的长度 word
    DMA_InitStructure.DMA_BufferSize = 112;		   // 定义指定 DMA 通道 DMA 缓存的大小
...
//float p_ch[16];
	...
	m = m/112;
	n = m;
	printf("The current temperature is %4.2f度   \n",n); 
